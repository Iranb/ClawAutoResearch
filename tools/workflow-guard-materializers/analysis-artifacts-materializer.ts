...
function buildClaimRows(params: {
  ...
  return [
    ...
    },
    {
      claim_id: "C3",
      claim:
        "The analysis should present the method contribution as a bounded consistency-filtering result until external benchmark runs replace the local reference benchmark.",
...
The evaluated change is ${params.oneChange}. The claim should stay scoped to the current local reference benchmark evidence and should not be presented as a full external benchmark result.
...
The remaining risk is external validity: the paper must label this as local reference benchmark evidence until the same method is rerun on benchmark suites.
...
The appendix should spell out the assumptions: fixed local reference split, shared known/novel evaluation, identical scoring code, and bounded local reference evidence.
`;
}
...
function buildProofPacket(params: {
  ...
  return {
    ...
    short_result:
      "The local reference benchmark supports using consistency agreement as a bounded noise-control mechanism for the known/novel balance.",
    body_safe: true,
    confidence: "local_reference",
    ...
    caveats: [
      "The current derivation is a mechanism explanation for local reference benchmark evidence.",
...
function buildTheoryState(params: {
  ...
  return {
    ...
    theorem_candidates: [
      {
        ...params.proofPacket,
        packet_id: "thm_fixmatch_gcd_reference_support",
        role: "theorem",
        title: "Bounded support for consistency-filtered GCD",
        statement:
          "Under the local reference benchmark contract, consistency-filtered pseudo-label expansion supports the paper's bounded empirical claim when execution proof and claim evidence remain aligned.",
        short_result:
    ...
    appendix_sections: [
      {
        ...
        title: "Consistency Filtering for Local GCD Reference Evidence",
        purpose:
...
export async function materializeAnalysisArtifactsImpl(params: {
  ...
  await writeJsonEnsured(path.join(projectRoot, "researcher", "ablation_summary.json"), {
    ...
    interpretation:
      "Ablations are treated as local reference controls for the consistency and debiasing components.",
  });
...
The appendix should keep assumptions explicit and should preserve the local reference benchmark evidence boundary.
`
  );
...
`
  ...
  await writeJsonEnsured(resolve(projectRoot, ANALYZER_FILES.mechanismEvidence), {
    ...
    generated_at: now,
    status: "ready",
    evidence_tier: "local_reference",
    graph_context_status: "ready",
    one_change_signature: oneChange,
    summary:
      "Consistency filtering is treated as a noise-control mechanism for the local GCD reference benchmark.",
    evidence_pointers: [
      "researcher/evaluation_summary.json",
  ...
  await writeJsonEnsured(resolve(projectRoot, ANALYZER_FILES.venueCompetition), {
    ...
    graph_context_status: "ready",
    summary:
      "The result is draftable as a bounded local reference contribution; external benchmark strength remains a review risk.",
    evidence_pointers: [
