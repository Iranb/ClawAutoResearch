...
export type ExperimentInnerLoopContract = {
  mode: string | null;
...
export type ExperimentOuterLoopPolicy = {
  requireBaselineDatasetCoverageForEffectiveCandidates: boolean;
  innovationDeviationTolerance: string | null;
};

export type BaselineDatasetCoverage = {
  status: "unknown" | "covered" | "partial" | "missing";
  baselineDatasets: string[];
  validatedDatasets: string[];
  missingDatasets: string[];
  summary: string | null;
};

export type InnovationDeviation = {
  status: "unknown" | "aligned" | "partial_drift" | "broad_drift";
  score: number | null;
  anchorPoints: string[];
  summary: string | null;
};
...
function collectTextTokens(text: string | null | undefined): Set<string> {
  return new Set(
    String(text ?? "")
      .toLowerCase()
      .split(/[^a-z0-9]+/g)
...
export function normalizeExperimentInnerLoopContract(
  specLike: unknown
): ExperimentInnerLoopContract {
  ...
  return {
    mode:
      normalizeStage(innerLoop.mode ?? record.inner_loop_mode ?? record.innerLoopMode) ??
      "karpathy_fast_keep_discard",
    trialTimeBudgetMinutes:
      pickNumber(innerLoop, [
        "trialTimeBudgetMinutes",
    ...
    keepDiscardRule:
      pickString(innerLoop, ["keepDiscardRule", "keep_discard_rule"]) ??
      "primary_metric_keep_discard",
  };
}

export function normalizeExperimentOuterLoopPolicy(
  specLike: unknown
): ExperimentOuterLoopPolicy {
  const record = asRecord(specLike) ?? {};
  ...
  return {
    requireBaselineDatasetCoverageForEffectiveCandidates:
      pickBoolean(outerLoop, [
        "requireBaselineDatasetCoverageForEffectiveCandidates",
        "require_baseline_dataset_coverage_for_effective_candidates",
      ]) ?? true,
    innovationDeviationTolerance:
      normalizeStage(
        outerLoop.innovationDeviationTolerance ??
          outerLoop.innovation_deviation_tolerance
      ) ?? "wide",
  };
}

export function buildOneChangeSignature(params: {
  trackRecords?: Array<Record<string, unknown>> | null;
}): string | null {
...
export function collectBaselineDatasetEnvelope(params: {
  ...
  return uniqueStrings([
    ...
    ...trackRecords.flatMap((track) =>
      asStringArray(track.datasets ?? track.dataset_scope ?? track.datasetScope)
    ),
  ]);
}
  ...
  return uniqueStrings(
    experiments
      ...
      .filter((entry) => {
        ...
        if (
          Array.isArray(params.experimentIds) &&
...
export function deriveBaselineDatasetCoverage(params: {
  ...
  const missingDatasets = baselineDatasets.filter(
    (entry) => !validatedDatasets.includes(entry)
  );
  ...
  if (validatedDatasets.length === 0) {
    return {
      ...
      missingDatasets,
      summary:
        "No validated dataset evidence is recorded yet, so baseline-dataset coverage cannot be judged.",
    };
  }
...
export function deriveMeasuredTrialDurationMinutes(params: {
  ledgerLike: unknown;
  preferredExperimentIds?: string[] | null;
}): number | null {
  const ledger = asRecord(params.ledgerLike) ?? {};
  const experiments = Array.isArray(ledger.experiments) ? ledger.experiments : [];
  ...
  const matching = experiments
    ...
    .sort((left, right) => {
      const leftTs = Date.parse(
        pickString(left, ["updatedAt", "updated_at", "completedAt", "completed_at"]) ??
          new Date(0).toISOString()
      );
      const rightTs = Date.parse(
        pickString(right, ["updatedAt", "updated_at", "completedAt", "completed_at"]) ??
          new Date(0).toISOString()
      );
...
export function deriveComparableTrialBudgetStatus(params: {
  innerLoop: ExperimentInnerLoopContract;
