...
export function normalizeReviewPressurePacketState(value) {
    const record = asRecord(value) ?? {};
    return {
        status: normalizeStage(record.status) ?? "missing",
        rejectFirstReviewPath: pickString(record, [
            "rejectFirstReviewPath",
            "reject_first_review_path",
        ]) ?? DEFAULT_REJECT_FIRST_REVIEW_PATH,
        noveltyAttackPath: pickString(record, ["noveltyAttackPath", "novelty_attack_path"]) ??
            DEFAULT_NOVELTY_ATTACK_PATH,
        unsupportedClaimAuditPath: pickString(record, [
            "unsupportedClaimAuditPath",
            "unsupported_claim_audit_path",
        ]) ?? DEFAULT_UNSUPPORTED_CLAIM_AUDIT_PATH,
        reverseOutlinePath: pickString(record, ["reverseOutlinePath", "reverse_outline_path"]) ??
            DEFAULT_REVERSE_OUTLINE_PATH,
        figureTableQcPath: pickString(record, ["figureTableQcPath", "figure_table_qc_path"]) ??
            DEFAULT_FIGURE_TABLE_QC_PATH,
        limitationAuditPath: pickString(record, ["limitationAuditPath", "limitation_audit_path"]) ??
...
export function serializeReviewPressurePacketState(value) {
    return {
        status: value.status,
