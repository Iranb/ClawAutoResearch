...
- `papers-cool` 作为稳定检索基线，`pasa-paper-search` 作为可选第二检索源；二者合并后在确认论文身份后优先走 `hugging-face-paper-pages`，再走 `arxiv2md-api`，再走 `markxiv`，最后才走 `arxiv2md` 的 Markdown-first 获取链路
...
`papers-cool search (+ optional pasa-paper-search merge) -> once paper identity is confirmed -> hugging-face-paper-pages Markdown -> arxiv2md-api direct markdown fallback -> markxiv markdown fallback -> arxiv2md legacy markdown fallback -> PDF fallback only if all Markdown sources are unavailable -> graph refresh / PaperNexus`
