...
function collectEvidencePointers(record: TrackLike): string[] {
  return uniqueStrings([
    ...
    ...asStringArray(record.bridge_path_ids ?? record.bridgePathIds).map(
      (entry) => `paper_nexus:bridge_path:${normalizeEvidencePointer(entry)}`
    ),
    ...normalizeObjectArrayStrings(
      record.evidence_chain_refs ?? record.evidenceChainRefs,
      ["ref_id", "refId", "node_id", "nodeId", "source", "reference", "ref"],
      16
    ).map((entry) => `paper_nexus:evidence_ref:${entry}`),
...
function collectLinkedGraphNodes(record: TrackLike): string[] {
  return uniqueStrings([
    ...
    ...normalizeObjectArrayStrings(
      record.evidence_chain_refs ?? record.evidenceChainRefs,
      ["node_id", "nodeId", "snippet_node_id", "snippetNodeId"],
      16
    ),
    ...normalizeObjectArrayStrings(record.source_spans ?? record.sourceSpans, [
      "snippet_node_id",
...
function collectRelationPatterns(record: TrackLike): string[] {
  return uniqueStrings([
    ...
    ...asStringArray(record.bridge_path_ids ?? record.bridgePathIds).map(
      (entry) => `paper_nexus_bridge_path:${entry}`
    ),
