...
├── EXPERIMENT_LEDGER.json          # structured experiment memory ledger template
...
- **PROJECT_MANIFEST.json**, **TRACK_REGISTRY.json**, **CLAIM_POLICY.md**: recommended to be copied into `{PROJ}/` during project initialization as the unified templates for the state machine, track portfolio, recovery, audit, writing evidence gates, and bounded `idle_research` state.
- **EXPERIMENT_LEDGER.json**, **EXPERIMENT_REGISTRY.md**, **IDEA_TOURNAMENT_STATE.json**, **PROJECTS_STATE.json**: copy into the project directory as needed (for example under `{PROJ}/researcher/` or `{PROJECTS_ROOT}/`) as initial state or reference format.
