...
test("llm sufficiency prompt includes unresolved questions and target domain", () => {
  ...
  assert.match(prompt, /source spans/i);
  assert.match(prompt, /confidence as grounding/i);
});
...
test("llm question generator prompt and parser support decomposition augmentation", () => {
  ...
  assert.match(prompt, /metacognition-driven interdisciplinary ideation/i);
  assert.match(prompt, /mechanism-level abstraction/i);
  assert.match(prompt, /Do not propose source domains/i);
