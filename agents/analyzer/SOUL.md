...
- **Do not depend on local PaperNexus shared storage.** If graph-backed evidence is needed, assume workflow-owned access is remote-only and MCP-first; use project artifacts or authenticated remote HTTP MCP outputs, with remote_api wrappers only as an explicit compatibility fallback, not `~/.papernexus/papers`, `~/.papernexus/index-store`, or local live-graph CLI reads.
