...
test("paper writing workbenches can read adjustable prompts from config file", async (t) => {
  const projectRoot = await makeWorkbenchProjectRoot();
  t.after(async () => {
    await fs.rm(projectRoot, { recursive: true, force: true });
  });
  const promptConfigPath = path.join(projectRoot, "workflow-prompts.config.json");
  await writeJson(promptConfigPath, {
    paperWriting: {
      titleAbstractIntroWorkbench: {
        titleTemplates: {
          experiment: [
            "Configured title for {{mechanism}}",
          ],
        },
        abstractWorkbench: {
          heading: "# Configured Abstract Workbench",
        },
        introWorkbench: {
          paragraph3Heading: "## Configured Mechanism Paragraph",
        },
      },
      resultsStoryline: {
        markdownHeading: "# Configured Results Prompt Order",
        experimentQuestions: {
          effectiveness: {
            prompt:
              "Configured result question for {{primaryMetric}} against {{baselineReference}}?",
            objective: "Configured objective from prompt JSON.",
          },
        },
      },
    },
  ...
  assert.equal(result.state.titleCandidates[0]?.title.startsWith("Configured title for "), true);
  assert.equal(result.state.status, "ready");
  ...
  assert.match(questionOrder, /# Configured Results Prompt Order/);
  assert.match(questionOrder, /Configured result question for support_precision against baseline-router/);
  assert.match(questionOrder, /Configured objective from prompt JSON/);
  assert.match(abstractWorkbench, /# Configured Abstract Workbench/);
  assert.match(introWorkbench, /## Configured Mechanism Paragraph/);
});
