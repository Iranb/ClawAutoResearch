...
export function formatWorkflowSnapshotForPromptImpl(
  ...
  if (snapshot.surveyReviewStatus) {
    ...
    lines.push(
      `Survey gates: coverage=${snapshot.surveyReviewCoverageStatus ?? "missing"}, taxonomy=${snapshot.surveyReviewTaxonomyStabilityStatus ?? "missing"}, representative_methods=${snapshot.surveyReviewRepresentativeMethodsStatus ?? "missing"}, benchmark_alignment=${snapshot.surveyReviewBenchmarkAlignmentStatus ?? "missing"}, topic_relevance=${snapshot.surveyReviewTopicRelevanceStatus ?? "missing"}, gap_closure=${snapshot.surveyReviewGapClosureStatus ?? "missing"}, blockers=${snapshot.surveyReviewGateBlockingIssueCount ?? 0}`
    );
