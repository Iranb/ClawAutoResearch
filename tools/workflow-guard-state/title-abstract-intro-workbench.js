import { asRecord, asStringArray, normalizeStage, pickString, } from "../workflow-guard-core/coercion";
export const DEFAULT_TITLE_CANDIDATES_PATH = "academic_writer/TITLE_CANDIDATES.md";
export const DEFAULT_ABSTRACT_WORKBENCH_PATH = "academic_writer/ABSTRACT_5_SENTENCE_WORKBENCH.md";
export const DEFAULT_INTRO_WORKBENCH_PATH = "academic_writer/INTRO_5_PARAGRAPH_WORKBENCH.md";
...
function serializeTitleCandidate(value) {
    return {
        title: value.title,
        focus: value.focus,
...
export function normalizeTitleAbstractIntroWorkbenchState(value) {
    const record = asRecord(value) ?? {};
    const candidatesRaw = Array.isArray(record.titleCandidates ?? record.title_candidates)
        ? (record.titleCandidates ?? record.title_candidates)
        : [];
    return {
        ...
        titleCandidatesPath: pickString(record, [
            "titleCandidatesPath",
            "title_candidates_path",
        ]) ?? DEFAULT_TITLE_CANDIDATES_PATH,
        abstractWorkbenchPath: pickString(record, [
            "abstractWorkbenchPath",
            "abstract_workbench_path",
        ]) ?? DEFAULT_ABSTRACT_WORKBENCH_PATH,
        introWorkbenchPath: pickString(record, [
            "introWorkbenchPath",
            "intro_workbench_path",
        ]) ?? DEFAULT_INTRO_WORKBENCH_PATH,
        alignmentStatus: normalizeStage(record.alignmentStatus ?? record.alignment_status) ?? "missing",
...
export function serializeTitleAbstractIntroWorkbenchState(value) {
    return {
        status: value.status,
        selected_title: value.selectedTitle,
        title_candidates: value.titleCandidates.map((entry) => serializeTitleCandidate(entry)),
