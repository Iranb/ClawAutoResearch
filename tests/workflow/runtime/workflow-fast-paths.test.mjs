...
async function writeReadyPaperArtifactProject(projectRoot, manifest) {
...
test("drainQueuedBackgroundWorkflowRuns does not consume stale paper artifacts before write stage", async (t) => {
  const workspaceRoot = await makeTempWorkspace();
  const projectsRoot = path.join(workspaceRoot, "projects");
  const projectRoot = path.join(projectsRoot, "prewrite-terminal-project");
  const queueKey = "prewrite-terminal-background-run";
  const runCalls = [];
  ...
  await writeReadyPaperArtifactProject(projectRoot, {
    project_id: "prewrite-terminal-project",
    current_stage: "graph_build",
    owner_agent: "researcher",
  });
  await enqueueQueuedBackgroundWorkflowRun({
    source: "start_background_run",
    ownerAgent: "researcher",
    requesterSessionKey: "agent:researcher:local:conversation:prewrite-terminal",
    messageChannel: "local",
    preferredSessionKey:
      "agent:researcher:local:conversation:prewrite-terminal:subagent:workflow-research-pipeline:prewrite",
    family: "research",
    kind: "research_pipeline",
    projectId: "prewrite-terminal-project",
    projectRoot,
    projectsRoot,
    queueKey,
    summary: "Replay pre-write work despite stale paper artifact.",
    runPayload: {
      message: "/research-pipeline prewrite",
      lane: "nested",
      deliver: false,
      idempotencyKey: null,
      extraSystemPrompt: null,
  ...
  const drained = await drainQueuedBackgroundWorkflowRuns({
    workflowRuntime: {
      async run(params) {
        runCalls.push(params);
        return { runId: "run-prewrite-terminal" };
      },
      async waitForRun() {
        return { status: "timeout" };
      },
  ...
  assert.equal(runCalls.length, 1);
  assert.equal(drained.started.length, 1);
  assert.equal(drained.started[0].queueKey, queueKey);
  const queue = await readWorkflowRuntimeQueueStore(projectRoot);
  assert.equal(queue.entries.find((entry) => entry.queueKey === queueKey)?.status, "running");
});

test("drainQueuedBackgroundWorkflowRuns consumes paper artifacts once workflow reaches write", async (t) => {
  const workspaceRoot = await makeTempWorkspace();
  const projectsRoot = path.join(workspaceRoot, "projects");
  const projectRoot = path.join(projectsRoot, "write-terminal-project");
  const queueKey = "write-terminal-background-run";
  const runCalls = [];
  ...
  await writeReadyPaperArtifactProject(projectRoot, {
    project_id: "write-terminal-project",
    current_stage: "write",
  ...
  await enqueueQueuedBackgroundWorkflowRun({
    source: "start_background_run",
    ownerAgent: "researcher",
    requesterSessionKey: "agent:researcher:local:conversation:write-terminal",
    messageChannel: "local",
    preferredSessionKey:
      "agent:researcher:local:conversation:write-terminal:subagent:workflow-research-pipeline:write",
    family: "research",
    kind: "research_pipeline",
    projectId: "write-terminal-project",
    projectRoot,
    projectsRoot,
    queueKey,
    summary: "Retire stale work after paper is ready.",
    runPayload: {
      message: "/research-pipeline write",
      lane: "nested",
      deliver: false,
      idempotencyKey: null,
      extraSystemPrompt: null,
  ...
  const drained = await drainQueuedBackgroundWorkflowRuns({
    workflowRuntime: {
      async run(params) {
        runCalls.push(params);
        return { runId: "run-write-terminal" };
      },
      async waitForRun() {
        return { status: "timeout" };
      },
  ...
  assert.equal(drained.started.length, 0);
  const queue = await readWorkflowRuntimeQueueStore(projectRoot);
  assert.equal(queue.entries.find((entry) => entry.queueKey === queueKey)?.status, "completed");
  const events = await readWorkflowRuntimeEvents(projectRoot);
  assert.equal(
    events.some(
      (event) =>
        event.kind === "background_queue_retired_project_terminal" &&
        event.details?.queueKey === queueKey
    ),
    true
  );
});
