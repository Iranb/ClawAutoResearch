...
test("research_workflow build_query_answer_list returns portable select presentation", async () => {
  ...
  const result = await executeWorkflowTool(tool, {
    ...
    queryAnswerList: {
      ...
      context: "The next step needs user preference before the workflow continues.",
      ...
      options: [
        ...
        },
        {
          ...
          value: "experiment",
          description: "Prioritize implementation and benchmark results.",
        },
      ],
    },
  });

  assert.equal(result.kind, "query_answer_list");
  assert.equal(result.schemaVersion, "query-answer-list-v1");
  assert.equal(result.channel, "discord");
  assert.equal(result.requesterAgent, "researcher");
  assert.match(result.message.text, /Which research direction/);
  assert.match(result.message.text, /Graph-grounded survey/);
  assert.match(result.message.text, /Experiment-first paper/);

  const selectBlock = result.message.presentation.blocks.find(
    (block) => block.type === "select"
  );
  assert.ok(selectBlock);
  assert.equal(selectBlock.placeholder, "Pick a direction");
  assert.deepEqual(selectBlock.options, [
    { label: "Graph-grounded survey", value: "answer:survey" },
    { label: "Experiment-first paper", value: "answer:experiment" },
  ]);
  assert.equal(result.query.optionCount, 2);
  assert.equal(result.query.truncatedOptionCount, 0);
});

test("research_workflow build_query_answer_list caps select options at 25", async () => {
  const tool = createResearchWorkflowTool();
  const result = await executeWorkflowTool(tool, {
    action: "build_query_answer_list",
    queryAnswerList: {
      question: "Choose one option",
      options: Array.from({ length: 30 }, (_unused, index) => ({
  ...
  assert.ok(selectBlock);
  assert.equal(selectBlock.options.length, 25);
  assert.equal(result.query.optionCount, 25);
  assert.equal(result.query.truncatedOptionCount, 5);
});

test("research_workflow build_query_answer_list rejects incomplete query answer lists", async () => {
  const tool = createResearchWorkflowTool();
  await assert.rejects(
    ...
    /queryAnswerList\.question is required/
  );
  await assert.rejects(
    () =>
      tool.execute("test-call", {
    ...
    /queryAnswerList\.options must include at least one option/
  );
});
