...
export type StorylinePlannerState = {
  status: string;
  configuredMode: string | null;
  activePrimaryMode: string | null;
...
export const DEFAULT_STORYLINE_PLANNER_CANDIDATES_PATH =
  "academic_writer/SURVEY_STORYLINE_CANDIDATES.json";
export const DEFAULT_STORYLINE_PLANNER_JUDGE_PACKET_PATH =
  "academic_writer/SURVEY_STORYLINE_JUDGE_PACKET.json";
export const DEFAULT_STORYLINE_PLANNER_SELECTION_PATH =
...
export function normalizeStorylinePlannerState(value: unknown): StorylinePlannerState {
  ...
  return {
    ...
    activePrimaryMode: pickString(record, [
      "activePrimaryMode",
      "active_primary_mode",
    ]),
    fallbackToHeuristic:
      pickBoolean(record, ["fallbackToHeuristic", "fallback_to_heuristic"]) ?? true,
    fallbackTriggered:
      pickBoolean(record, ["fallbackTriggered", "fallback_triggered"]) ?? false,
    fallbackReason: pickString(record, ["fallbackReason", "fallback_reason"]),
    shadowMode: pickString(record, ["shadowMode", "shadow_mode"]),
    shadowDiffStatus:
      normalizeStage(record.shadowDiffStatus ?? record.shadow_diff_status) ?? null,
    selectionConfidence:
      typeof pickNumber(record, ["selectionConfidence", "selection_confidence"]) === "number"
        ? Math.max(
            0,
            Math.min(
              1,
              pickNumber(record, ["selectionConfidence", "selection_confidence"]) ?? 0
            )
          )
        : null,
    candidatePath:
    ...
    judgePacketPath:
      pickString(record, ["judgePacketPath", "judge_packet_path"]) ??
      DEFAULT_STORYLINE_PLANNER_JUDGE_PACKET_PATH,
    selectionPath:
      pickString(record, ["selectionPath", "selection_path"]) ??
...
export function serializeStorylinePlannerState(
  value: StorylinePlannerState
