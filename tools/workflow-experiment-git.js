...
async function resolveCommit(projectRoot, ref) {
    if (!ref?.trim()) {
        return null;
    }
    try {
...
async function ensureBranchNotCheckedOut(projectRoot, branch) {
    if (!branch) {
        return;
    }
    ...
    if (active) {
        throw new Error(`Branch ${branch} is currently checked out in worktree ${active.worktree}; workflow-owned ref promotion requires the incumbent branch to be detached from active worktrees.`);
    }
}
async function ensureAncestor(projectRoot, ancestor, descendant) {
...
export async function executeExperimentGitAction(params) {
    ...
    if (params.actionType === "create_candidate_worktree") {
        ...
        const baseCommit = (await resolveCommit(params.projectRoot, baseRef)) ??
        ...
        const branchExists = await refExists(params.projectRoot, `refs/heads/${candidateBranch}`);
        const worktrees = await listWorktrees(params.projectRoot);
        const existingWorktree = worktrees.find((entry) => entry.worktree === candidateWorktreePath);
        if (existingWorktree?.branch === candidateBranch) {
            return {
                ...
                candidateHeadCommit: existingWorktree.head ??
    ...
    const candidateCommit = params.candidateHeadCommit?.trim() ||
        (await resolveCommit(params.projectRoot, candidateBranch));
    if (!candidateCommit) {
        throw new Error(`Unable to resolve candidate branch ${candidateBranch}.`);
    }
    if (params.actionType === "promote_candidate") {
        const incumbentCommit = (await resolveCommit(params.projectRoot, incumbentBranch)) ??
        ...
        if (params.discardCandidateBranchAfterPromote !== false &&
        ...
        return {
            ...
            incumbentCommit: await resolveCommit(params.projectRoot, incumbentBranch),
            candidateBranch,
            candidateBaseCommit: params.incumbentCommit?.trim() ??
