...
- **PaperNexus is remote-only in workflow-owned work.** Do not read or write `~/.papernexus/papers` or `~/.papernexus/index-store`, and do not rely on local live-graph CLI operations against workflow state. Use `{PROJ}/researcher/paper-staging/`, queue upload work through `research_workflow.queue_paper_ingestion`, and reserve `research_workflow.run_papernexus_wrapper` for live graph / brainstorm wrappers with the authenticated Python wrapper set (`pn_stage_sync.py`, `pn_import_submit.py`, `pn_import_queue.py`, `pn_batch_import.py`, `pn_graph_query.py`, `pn_research_chains.py`).
