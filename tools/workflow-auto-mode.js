...
function normalizeRiskFingerprintSignal(signal, stage) {
    const normalized = signal.trim();
    if (/graph_presence_status|GRAPH_PRESENCE|missing canonical papers|graph_source_dir|paper_source_dir|PaperNexus corpus still misses canonical papers/i.test(normalized)) {
    ...
    if (/brainstorm_cycle\.|researcher\/brainstorm-cycle\/|TOPIC_SUMMARY|RESEARCH_BRIEF|BRAINSTORM_BRIEF|WORKING_MEMORY|LOGIC_CHAIN|EVIDENCE_CHAIN|REASONING_TRACE|QUESTION_PACKET|SYNTHESIS_PACKET/i.test(normalized)) {
        return "brainstorm_cycle_outputs_incomplete";
    }
    if (stage === "idea" &&
        /IDEA_REPORT|IDEA_AUDIT|ideation_contract\.|researcher\/ideation\/|researcher\/idea-catalyst\/|TRACK_REGISTRY/i.test(normalized)) {
        return "ideation_stage_outputs_incomplete";
    }
    return null;
}
function normalizeRiskFingerprintReason(reason) {
    const normalized = reason.trim();
    if (/^Workflow regressed to an earlier stage\.$/i.test(normalized)) {
        return null;
    }
    if (/Graph presence is|Paper ingestion indicates the graph needs refresh|Graph-related stage signals are missing/i.test(normalized)) {
        return "graph_presence_missing";
    }
...
export function buildWorkflowAutoModeRiskFingerprint(params) {
    ...
    return createHash("sha1")
        ...
        reasons: [
            ...new Set(params.reasons
                .map(normalizeRiskFingerprintReason)
                .filter((entry) => Boolean(entry))),
        ].sort(),
        missingStageSignals: [
            ...new Set((params.missingStageSignals ?? [])
