...
test("focused prompt assembly can read adjustable prompt rules from config file", async (t) => {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workflow-prompt-config-"));
  t.after(async () => {
    await fs.rm(tempRoot, { recursive: true, force: true });
  });
  const promptConfigPath = path.join(tempRoot, "workflow-prompts.config.json");
  await writeJson(promptConfigPath, {
    workflowGuard: {
      focused: {
        stablePolicyRules: [
          "Custom stable policy for {{role}} while owner is {{owner}}.",
        ],
        academicWriterRules: [
          "Custom writing-stage prompt configured outside TypeScript.",
        ],
      },
    },
  });
  ...
  const assembly = buildFocusedPromptAssembly({
    ...
    snapshot: {
      ...
      ownerAgent: "academic_writer",
      recommendedOwner: "academic_writer",
      writingCurrentSection: "method",
    },
  });

  assert.match(assembly.text, /Custom stable policy for academic_writer/i);
  assert.match(assembly.text, /owner is academic_writer/i);
  assert.match(assembly.text, /Custom writing-stage prompt configured outside TypeScript/i);
});
