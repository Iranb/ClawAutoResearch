...
- If analysis uses PaperNexus graph or reflection artifacts, honor workflow access mode:
  - `remote_mcp`: use the configured remote PaperNexus HTTP MCP endpoint for graph-backed analysis tasks; prefer `research_lookup`, `research_briefing`, and `idea_catalyst`.
  - `remote_api`: expect authenticated wrapper / Web API artifacts only as compatibility-mode inputs, not as the preferred live-graph control plane.
  - `local_mcp`: use PaperNexus MCP tools for graph-backed analysis tasks only when the workflow explicitly routes that way.
  - `auto`: prefer remote HTTP MCP first, then remote_api compatibility artifacts, and only fall back to local MCP when workflow guidance explicitly allows it.
