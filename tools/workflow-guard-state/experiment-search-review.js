import { asStringArray, normalizeStage, pickBoolean, pickNumber, pickString, } from "../workflow-guard-core/coercion";
export const DEFAULT_EXPERIMENT_SEARCH_REVIEW_STATE_PATH = "researcher/EXPERIMENT_SEARCH_GIT_REVIEW_STATE.json";
export const DEFAULT_EXPERIMENT_SEARCH_REVIEW_PACKET_PATH = "planner/EXPERIMENT_SEARCH_GIT_REVIEW_PACKET.json";
export const DEFAULT_EXPERIMENT_SEARCH_REVIEW_PLAN_PATH = "planner/EXPERIMENT_SEARCH_GIT_PLAN.md";
export const DEFAULT_EXPERIMENT_SEARCH_ANALYZER_REPORT_PATH = "analyzer/EXPERIMENT_SEARCH_GIT_REASONABLENESS_REPORT.md";
export const DEFAULT_EXPERIMENT_SEARCH_CROSS_REVIEWER_REPORT_PATH = "cross-reviewer/EXPERIMENT_SEARCH_GIT_ATTACK_REPORT.md";
export const DEFAULT_EXPERIMENT_SEARCH_DECISION_PATH = "researcher/EXPERIMENT_SEARCH_GIT_DECISION.json";
export function normalizeExperimentSearchReviewVerdict(value) {
...
export function coerceCompletedSearchReviewStatus(status, verdict, currentStatus = "pending") {
...
export function normalizeExperimentSearchReviewState(value) {
    ...
    return {
        ...
        actionStatus: normalizeStage(record.actionStatus ?? record.action_status) ?? "requested",
        stateFilePath: pickString(record, ["stateFilePath", "state_file_path"]) ??
        ...
        plannerPlanPath: pickString(record, ["plannerPlanPath", "planner_plan_path"]) ??
            DEFAULT_EXPERIMENT_SEARCH_REVIEW_PLAN_PATH,
        analyzerReportPath: pickString(record, ["analyzerReportPath", "analyzer_report_path"]) ??
            DEFAULT_EXPERIMENT_SEARCH_ANALYZER_REPORT_PATH,
        crossReviewerReportPath: pickString(record, [
            "crossReviewerReportPath",
            "cross_reviewer_report_path",
        ]) ?? DEFAULT_EXPERIMENT_SEARCH_CROSS_REVIEWER_REPORT_PATH,
        decisionPath: pickString(record, ["decisionPath", "decision_path"]) ??
        ...
        plannerStatus: normalizeStage(record.plannerStatus ?? record.planner_status) ?? "pending",
        analyzerStatus: normalizeStage(record.analyzerStatus ?? record.analyzer_status) ?? "pending",
        crossReviewerStatus: normalizeStage(record.crossReviewerStatus ?? record.cross_reviewer_status) ??
            "pending",
        synthesisStatus: normalizeStage(record.synthesisStatus ?? record.synthesis_status) ??
            "pending",
        ...
        actionApproved: pickBoolean(record, ["actionApproved", "action_approved"]) ?? false,
        blockerCount: Math.max(0, Math.floor(pickNumber(record, ["blockerCount", "blocker_count"]) ?? 0)),
        blockers: asStringArray(record.blockers),
        promotionBasisSignals: asStringArray(record.promotionBasisSignals ?? record.promotion_basis_signals),
...
export function serializeExperimentSearchReviewState(value) {
    return {
        status: value.status,
