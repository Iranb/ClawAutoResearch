...
export async function captureWorkflowDiagnosticBundle(params: {
  ...
  const extraArtifacts = await Promise.all([
    ...
    captureOptionalFile({
      projectRoot,
      bundleDir,
      relativePath: "reviewer/REVISION_CONTROL_PACKET.json",
    }),
    captureOptionalFile({
      projectRoot,
      bundleDir,
      relativePath: "reviewer/REVISION_CONTROL_PACKET.md",
    }),
    captureOptionalFile({
      projectRoot,
      bundleDir,
      relativePath: "researcher/EXECUTION_PROOF.json",
    }),
    captureOptionalFile({
      projectRoot,
      bundleDir,
      relativePath: "academic_writer/PARAGRAPH_LOGIC_AUDIT.json",
    }),
    captureOptionalFile({
      projectRoot,
      bundleDir,
      relativePath: "academic_writer/PARAGRAPH_LOGIC_AUDIT.md",
    }),
    captureOptionalFile({
      projectRoot,
      bundleDir,
      relativePath: "academic_writer/PARAGRAPH_LOGIC_REVERSE_OUTLINE.md",
    }),
    captureOptionalFile({
      projectRoot,
      bundleDir,
      relativePath: "academic_writer/SURVEY_VISUAL_COMPILER_STATE.json",
    }),
    captureOptionalFile({
      projectRoot,
      bundleDir,
      relativePath: "academic_writer/SURVEY_VISUAL_INSERTION_MAP.json",
    }),
    captureOptionalFile({
      projectRoot,
      bundleDir,
      relativePath: "researcher/SURVEY_METHODOLOGY_CONSISTENCY.json",
    }),
    captureOptionalFile({
      projectRoot,
      bundleDir,
  ...
  const summary = [
    ...
    `- revision_control: ${snapshot.revisionControlStatus ?? "idle"}`,
    `- revision_open_sources: ${snapshot.revisionControlOpenSourceCount ?? 0}`,
    `- paragraph_logic_audit: ${snapshot.paragraphLogicAuditStatus ?? "unset"}`,
    `- paragraph_logic_blocking_issues: ${snapshot.paragraphLogicAuditBlockingIssueCount ?? 0}`,
    `- paragraph_logic_section_transitions: ${snapshot.paragraphLogicAuditSectionTransitionIssueCount ?? 0}`,
    `- execution_proof: ${snapshot.executionProofStatus ?? "unset"}`,
    `- execution_proof_receipts: ${snapshot.executionProofReceiptCount ?? 0}`,
    `- execution_proof_lineage_matched: ${snapshot.executionProofLineageMatchedReceiptCount ?? 0}`,
    `- execution_candidate_commit: ${snapshot.executionProofCandidateCommit ?? "unset"}`,
    `- execution_expected_stage_run_id: ${snapshot.executionProofExpectedStageRunId ?? "unset"}`,
    `- execution_receipt_experiment: ${snapshot.executionProofPrimaryReceiptExperimentId ?? "unset"}`,
    `- execution_receipt_run_id: ${snapshot.executionProofPrimaryReceiptRunId ?? "unset"}`,
    `- execution_receipt_stage_run_id: ${snapshot.executionProofPrimaryReceiptStageRunId ?? "unset"}`,
    `- execution_receipt_git_commit: ${snapshot.executionProofPrimaryReceiptGitCommit ?? "unset"}`,
    `- auto_dispatch_diagnostics: ${snapshot.autoDispatchDiagnosticsStatus ?? "unknown"}`,
    `- auto_dispatch_layer: ${snapshot.autoDispatchBlockingLayer ?? "unset"}`,
    `- graph_presence: ${snapshot.graphPresenceStatus ?? "unknown"}`,
    `- paper_ingestion_runtime: ${snapshot.paperIngestionRuntimeStatus ?? "unknown"}`,
    `- survey_visual_compiler: ${snapshot.surveyVisualCompilerStatus ?? "unset"}`,
    `- survey_methodology_consistency: ${snapshot.surveyMethodologyConsistencyStatus ?? "unset"}`,
