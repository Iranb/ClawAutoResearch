...
test("runWorkflowRuntimeMaintenancePass does not retire stale paper artifacts before write stage", async (t) => {
  const projectRoot = await makeProjectRoot();
  const dispatchQueueKey = "stale-terminal-paper::plan::orchestrator";
  ...
  const result = await runWorkflowRuntimeMaintenancePass({
    ...
    workflowRuntime: {
      async run() {
        throw new Error("pre-write paper artifacts must not replay dispatch");
      },
    },
    maxRepairAttempts: 3,
    staleSessionAgeMs: 365 * 24 * 60 * 60 * 1000,
