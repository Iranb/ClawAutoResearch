...
function sortDesc(values, getTime) {
    return [...values].sort((left, right) => (getTime(right) ?? "").localeCompare(getTime(left) ?? ""));
}
function getEntryTimestamp(entry) {
    return (pickString(entry, ["updatedAt", "updated_at"]) ??
...
function buildComparableRun(entry) {
    const metadata = getEntryMetadata(entry);
    const searchGit = asRecord(metadata?.searchGit ?? metadata?.search_git);
    return {
        experiment_id: pickString(entry, ["experimentId", "experiment_id"]) ?? "unknown-experiment",
        ...
        git_lineage: {
            ...
            retained: searchGit?.retained === true ||
...
function buildExperimentMemoryPacket(params) {
    ...
    const successful = ordered.filter((entry) => ["advance", "keep"].includes((pickString(entry, ["decision"]) ?? "").toLowerCase()));
    const failed = ordered.filter((entry) => ["discard", "failed", "timeout", "stalled", "killed", "cancelled"].includes((pickString(entry, ["decision", "status"]) ?? "").toLowerCase()));
    ...
    const graphRefs = uniqueStrings(ordered.flatMap((entry) => {
    ...
    const openFrontiers = uniqueStrings([
        ...asStringArray(searchRecord?.frontierExperimentIds ?? searchRecord?.frontier_experiment_ids),
    ...
    return {
        ...
        do_not_repeat_constraints: uniqueStrings(failed
            .flatMap((entry) => [
            pickString(entry, ["failureSignature", "failure_signature"]),
            ...asStringArray(entry.notes),
        ])
            .filter((entry) => Boolean(entry))).slice(0, 12),
        open_frontiers: openFrontiers,
        graph_refs: graphRefs,
        last_materialized_at: params.now,
        reflected_through_experiment_update_at: pickString(params.ledger, ["updatedAt", "updated_at"]) ?? null,
    };
}
function buildExperimentMemorySyncStatus(params) {
    ...
    const terminal = experiments
        ...
        : null)
        .filter((entry) => Boolean(entry))
        .filter((entry) => ["done", "failed", "timeout", "stalled", "killed", "cancelled", "merged", "completed"].includes((pickString(entry, ["status"]) ?? "").toLowerCase()));
    ...
    return {
        ...
        pending_reason: pendingExperimentIds.length > 0
            ? "terminal experiment outcomes still need PaperNexus/graph reconciliation"
            : null,
    };
}
export async function materializeExperimentMemoryPacketImpl(params, deps) {
    ...
    const syncStatusPath = pickString(patch, ["syncStatusPath", "sync_status_path"]) ??
        pickString(manifestMemory, ["graph_memory_sync_status_path"]) ??
        "researcher/papernexus/EXPERIMENT_MEMORY_SYNC_STATUS.json";
    const trackId = pickString(patch, ["trackId", "track_id"]) ?? searchState.trackId ?? null;
    const packet = buildExperimentMemoryPacket({
        trackId,
        baselineReference: pickString(manifest, ["baseline_reference"]) ??
            pickString(manifestResearchProgram, [
                "baseline_reference",
                "baselineReference",
            ]),
        primaryMetric: pickString(manifest, ["primary_metric"]) ??
            pickString(manifestResearchProgram, [
                "primary_metric",
                "primaryMetric",
            ]),
    ...
    return {
        ...
        packet,
        syncStatus,
        searchStatePath: updatedSearchState.searchStatePath ?? "researcher/EXPERIMENT_SEARCH.json",
    };
}
