...
export const DEFAULT_INNOVATION_SYNTHESIS_MEMO_PATH = "academic_writer/INNOVATION_SYNTHESIS_MEMO.md";
export const DEFAULT_INNOVATION_SYNTHESIS_GRAPH_PATH = "academic_writer/INNOVATION_SYNTHESIS_GRAPH.json";
export const DEFAULT_INTEGRATED_CONTRIBUTION_STATEMENT_PATH = "academic_writer/INTEGRATED_CONTRIBUTION_STATEMENT.md";
export const DEFAULT_STORY_GAP_SEARCH_PACKET_PATH = "researcher/story-gap-search/STORY_GAP_SEARCH_REQUISITION.json";
export const DEFAULT_STORY_GAP_SEARCH_BATCH_MANIFEST_PATH = "researcher/story-gap-search/batch-import.json";
function normalizeInnovationPoint(value, fallbackIndex) {
    const record = asRecord(value);
    if (!record) {
        return null;
    }
    const id = pickString(record, ["id", "pointId", "point_id"]) ??
    ...
    return {
        ...
        requiredForThesis: pickBoolean(record, ["requiredForThesis", "required_for_thesis"]) ?? true,
...
function serializeInnovationPoint(value) {
    return {
        id: value.id,
        claim: value.claim,
...
function normalizeSearchQuestion(value, fallbackIndex) {
    const record = asRecord(value);
    if (!record) {
        return null;
    }
    return {
        ...
        coverageStatus: normalizeStage(record.coverageStatus ?? record.coverage_status) ?? "unexplored",
        preferredSourceDomains: asStringArray(record.preferredSourceDomains ?? record.preferred_source_domains),
        candidateSourceDomains: asStringArray(record.candidateSourceDomains ?? record.candidate_source_domains),
        excludedAdjacentDomains: asStringArray(record.excludedAdjacentDomains ?? record.excluded_adjacent_domains),
        validationSampleSizeDefault: Math.max(1, Math.floor(pickNumber(record, [
            "validationSampleSizeDefault",
            "validation_sample_size_default",
        ]) ?? 20)),
        pruneIfIrrelevantRatioExceedsDefault: Math.max(0, Math.min(1, pickNumber(record, [
            "pruneIfIrrelevantRatioExceedsDefault",
            "prune_if_irrelevant_ratio_exceeds_default",
        ]) ?? 0.5)),
        minimumRelevantHits: Math.max(1, Math.floor(pickNumber(record, ["minimumRelevantHits", "minimum_relevant_hits"]) ?? 5)),
        nicheTopicOverrideAllowed: pickBoolean(record, ["nicheTopicOverrideAllowed", "niche_topic_override_allowed"]) ??
            true,
        minimumSources: Math.max(1, Math.floor(pickNumber(record, ["minimumSources", "minimum_sources"]) ?? 2)),
    };
}
function serializeSearchQuestion(value) {
    return {
        ...
        prune_if_irrelevant_ratio_exceeds_default: value.pruneIfIrrelevantRatioExceedsDefault,
...
export function normalizeInnovationSynthesisState(value) {
    const record = asRecord(value) ?? {};
    const innovationPointsRaw = Array.isArray(record.innovationPoints ?? record.innovation_points)
        ? (record.innovationPoints ?? record.innovation_points)
        : [];
    return {
        status: normalizeStage(record.status) ?? "missing",
        centralThesis: pickString(record, ["centralThesis", "central_thesis"]),
        integrationPattern: normalizeStage(record.integrationPattern ?? record.integration_pattern) ?? null,
        ...
        searchGapCount: Math.max(0, Math.floor(pickNumber(record, ["searchGapCount", "search_gap_count"]) ?? 0)),
...
export function serializeInnovationSynthesisState(value) {
    return {
        status: value.status,
        central_thesis: value.centralThesis,
        integration_pattern: value.integrationPattern,
        innovation_points: value.innovationPoints.map((entry) => serializeInnovationPoint(entry)),
...
export function normalizeStoryGapSearchRequisitionState(value) {
    const record = asRecord(value) ?? {};
    const targetQuestionsRaw = Array.isArray(record.targetQuestions ?? record.target_questions)
        ? (record.targetQuestions ?? record.target_questions)
        : [];
    return {
        status: normalizeStage(record.status) ?? "missing",
        ...
        executionMode: normalizeStage(record.executionMode ?? record.execution_mode) ?? null,
        ...
        sameGapCycleBudget: Math.max(0, Math.floor(pickNumber(record, ["sameGapCycleBudget", "same_gap_cycle_budget"]) ?? 2)),
        sameGapCyclesUsed: Math.max(0, Math.floor(pickNumber(record, ["sameGapCyclesUsed", "same_gap_cycles_used"]) ?? 0)),
        targetQuestions: targetQuestionsRaw
            .map((entry, index) => normalizeSearchQuestion(entry, index))
            .filter((entry) => Boolean(entry)),
        requiredStageReentry: asStringArray(record.requiredStageReentry ?? record.required_stage_reentry),
...
export function serializeStoryGapSearchRequisitionState(value) {
    return {
        status: value.status,
