...
2. **For EACH paper found** (ALL, not just selected few):
   ...
   - **Step 10:** For one staged paper, use `python3 scripts/pn_import_submit.py` plus `python3 scripts/pn_import_queue.py`; for 2 or more staged papers, create one manifest and use `python3 scripts/pn_batch_import.py`. When the task is primarily “build / track one manifest-backed multi-paper sync”, prefer the dedicated `/papernexus-batch-import` skill over ad hoc shell orchestration
