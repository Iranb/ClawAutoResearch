...
import { pathExists, readJsonIfExists, readTextIfExists, writeJsonEnsured, writeTextEnsured, } from "../workflow-guard-core/fs";
import { asRecord, asStringArray, pickString, } from "../workflow-guard-core/coercion";
import { loadExperimentSearchState, } from "../workflow-guard-experiment-history";
import { buildExperimentSearchReviewPacketFingerprint, loadExperimentSearchReviewState, saveExperimentSearchReviewStateFile, } from "../workflow-auto-experiment-search-review.js";
import { DEFAULT_EXPERIMENT_SEARCH_DECISION_PATH, DEFAULT_EXPERIMENT_SEARCH_REVIEW_PACKET_PATH, DEFAULT_EXPERIMENT_SEARCH_REVIEW_PLAN_PATH, coerceCompletedSearchReviewStatus, normalizeExperimentSearchReviewVerdict, } from "../workflow-guard-state/experiment-search-review.js";
import { normalizeExperimentSearchSpec, resolveExperimentSearchSpecPath, DEFAULT_EXPERIMENT_SEARCH_SPEC_PATH, } from "../workflow-guard-state/experiment-search-spec.js";
import { normalizeExperimentInnerLoopContract, normalizeExperimentOuterLoopPolicy, } from "../workflow-experiment-loop.js";
...
function defaultCandidateWorktreePath(params) {
    return path.join(os.tmpdir(), "openclaw-worktrees", sanitizePathPart(path.basename(params.projectRoot), "project"), sanitizePathPart(params.searchSessionId, "search-session"), sanitizePathPart(params.experimentId, "candidate"));
}
...
function extractCriticalBlockers(text) {
    if (!text) {
        return [];
    }
    const explicit = [...text.matchAll(/^\s*[-*]\s*(.+)$/gm)]
        ...
        .filter(Boolean);
    const criticalSection = text.match(/(?:critical(?:\s+blockers?|\s+issues?)|blockers?)\s*[:\n]([\s\S]{0,1200})/i)?.[1];
    const sectionBullets = criticalSection
        ? [...criticalSection.matchAll(/^\s*[-*]\s*(.+)$/gm)].map((match) => match[1]?.trim() ?? "")
        : [];
    return uniqueStrings([...sectionBullets, ...explicit].filter(Boolean)).slice(0, 8);
}
function readDecisionJson(value) {
    const record = value ?? {};
    return {
        actionApproved: record.action_approved === true || record.actionApproved === true,
        ...
        promotionBasisSignals: asStringArray(record.promotionBasisSignals ??
...
function deriveStatus(state) {
    ...
    if (state.crossReviewerVerdict === "block" ||
        state.analyzerVerdict === "block") {
        return "blocked";
    }
    if (state.crossReviewerVerdict === "revise" ||
...
async function writeJsonIfChanged(targetPath, value) {
...
async function writeTextIfChanged(targetPath, value) {
...
export async function materializeExperimentSearchReviewStateImpl(params, deps) {
    ...
    const specRaw = await readJsonIfExists(specResolvedPath);
    ...
    const actionType = pickString(patch, ["actionType", "action_type"]) ?? current.actionType ?? null;
    const experimentId = pickString(patch, ["experimentId", "experiment_id"]) ??
    ...
    const candidateBranch = buildCandidateBranch({
        ...
        experimentId,
        currentCandidateBranch: pickString(patch, ["candidateBranch", "candidate_branch"]) ??
            current.candidateBranch ??
            searchState.lastCandidateBranch,
    });
    const candidateWorktreePath = pickString(patch, ["candidateWorktreePath", "candidate_worktree_path"]) ??
    ...
    const incumbentBranch = pickString(patch, ["incumbentBranch", "incumbent_branch"]) ??
        current.incumbentBranch ??
        searchState.incumbentBranch ??
        spec.gitStrategy.incumbentBranch;
    const incumbentCommit = pickString(patch, ["incumbentCommit", "incumbent_commit"]) ??
        current.incumbentCommit ??
        searchState.incumbentCommit;
    const packetPath = pickString(patch, ["packetPath", "packet_path"]) ??
        current.packetPath ??
        DEFAULT_EXPERIMENT_SEARCH_REVIEW_PACKET_PATH;
    const plannerPlanPath = pickString(patch, ["plannerPlanPath", "planner_plan_path"]) ??
        current.plannerPlanPath ??
        DEFAULT_EXPERIMENT_SEARCH_REVIEW_PLAN_PATH;
    const analyzerReportPath = pickString(patch, ["analyzerReportPath", "analyzer_report_path"]) ??
        current.analyzerReportPath;
    const crossReviewerReportPath = pickString(patch, [
        "crossReviewerReportPath",
        "cross_reviewer_report_path",
    ]) ?? current.crossReviewerReportPath;
    const decisionPath = pickString(patch, ["decisionPath", "decision_path"]) ??
        current.decisionPath ??
        DEFAULT_EXPERIMENT_SEARCH_DECISION_PATH;
    const requestSummary = pickString(patch, ["requestSummary", "request_summary", "summary"]) ??
        ...
        (actionType === "create_candidate_worktree"
            ...
            : actionType === "promote_candidate"
                ...
                : actionType === "discard_candidate"
                    ...
                    : null);
    const packet = {
        action_id: pickString(patch, ["actionId", "action_id"]) ?? current.actionId ?? randomUUID(),
        action_type: actionType,
        trigger: params.trigger ?? "materialize_experiment_search_review_state",
        requested_by: pickString(patch, ["requestedBy", "requested_by"]) ??
            params.agentId ??
            current.requestedBy,
        requested_at: pickString(patch, ["requestedAt", "requested_at"]) ??
            current.requestedAt ??
            new Date().toISOString(),
        request_summary: requestSummary,
        project_id: pickString(manifest, ["project_id", "projectId"]) ?? spec.projectId ?? null,
        track_id: pickString(patch, ["trackId", "track_id"]) ??
            current.trackId ??
            searchState.trackId ??
            spec.trackId,
        experiment_id: experimentId,
        search_session_id: searchState.searchSessionId ?? spec.searchSessionId,
        search_spec_path: searchState.searchSpecPath ?? specResolvedPath.replace(`${params.projectRoot}/`, ""),
        search_state_path: searchState.searchStatePath ??
        ...
        candidate_commit: pickString(patch, ["candidateCommit", "candidate_commit"]) ??
        ...
        outer_loop: {
            require_baseline_dataset_coverage_for_effective_candidates: outerLoop.requireBaselineDatasetCoverageForEffectiveCandidates,
        ...
        git_strategy: {
            ...
            require_clean_candidate_history: spec.gitStrategy.requireCleanCandidateHistory,
            promotion_commit_policy: spec.gitStrategy.promotionCommitPolicy,
            discard_unpromoted_candidates: spec.gitStrategy.discardUnpromotedCandidates,
        },
    };
    ...
    const analyzerReportResolvedPath = analyzerReportPath && path.isAbsolute(analyzerReportPath)
        ? analyzerReportPath
        : analyzerReportPath
            ? path.join(params.projectRoot, analyzerReportPath)
            : null;
    const crossReviewerReportResolvedPath = crossReviewerReportPath && path.isAbsolute(crossReviewerReportPath)
        ? crossReviewerReportPath
        : crossReviewerReportPath
    ...
    const plannerPlanChanged = await writeTextIfChanged(plannerPlanResolvedPath, buildPlannerPlanMarkdown({
        actionType,
        experimentId,
        trackId: pickString(packet, ["track_id"]) ?? pickString(current, ["trackId", "track_id"]),
        incumbentBranch,
        candidateBranch,
    ...
    const crossText = crossReviewerReportResolvedPath
        ...
        : null;
    const decisionJson = await readJsonIfExists(decisionResolvedPath);
    ...
    const next = {
        ...
        actionType,
        actionStatus: pickString(patch, ["actionStatus", "action_status"]) ??
        ...
        crossReviewerReportPath: crossReviewerReportPath ?? current.crossReviewerReportPath,
        decisionPath,
        packetFingerprint,
        searchSessionId: pickString(packet, ["search_session_id"]) ?? current.searchSessionId,
        searchSpecPath: pickString(packet, ["search_spec_path"]) ?? current.searchSpecPath,
        searchStatePath: pickString(packet, ["search_state_path"]) ?? current.searchStatePath,
        trackId: pickString(packet, ["track_id"]) ?? current.trackId,
        experimentId,
        incumbentBranch,
        incumbentCommit,
        candidateBranch,
        candidateCommit: pickString(packet, ["candidate_commit"]) ?? current.candidateCommit,
        candidateWorktreePath,
        requestSummary,
        requestedBy: pickString(packet, ["requested_by"]) ??
            current.requestedBy ??
            params.agentId ??
            null,
        requestedAt: pickString(packet, ["requested_at"]) ??
            current.requestedAt ??
            new Date().toISOString(),
        plannerStatus: resetReviews
            ? "pending"
            : coerceCompletedSearchReviewStatus(current.plannerStatus, null, current.plannerStatus),
        analyzerStatus: resetReviews
            ? "pending"
            : coerceCompletedSearchReviewStatus(current.analyzerStatus, extractReviewVerdict(analyzerText), current.analyzerStatus),
        crossReviewerStatus: resetReviews
            ? "pending"
            : coerceCompletedSearchReviewStatus(current.crossReviewerStatus, extractReviewVerdict(crossText), current.crossReviewerStatus),
        synthesisStatus: resetReviews
            ? "pending"
        ...
        promotionBasisSignals: resetReviews
            ...
            : asStringArray(patch.promotionBasisSignals ??
    ...
    return {
        state: next,
        stateFilePath: path.join(params.projectRoot, next.stateFilePath ?? "researcher/EXPERIMENT_SEARCH_GIT_REVIEW_STATE.json"),
        stateFileExists: await pathExists(path.join(params.projectRoot, next.stateFilePath ?? "researcher/EXPERIMENT_SEARCH_GIT_REVIEW_STATE.json")),
        packetResolvedPath,
