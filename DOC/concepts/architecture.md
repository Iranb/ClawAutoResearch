...
- `papers-cool` 检索 + 确认论文身份后立即走 `hugging-face-paper-pages` 的 Markdown-first 获取链路
...
`papers-cool search -> once paper identity is confirmed -> hugging-face-paper-pages Markdown -> PDF fallback only if Markdown is unavailable -> graph refresh / PaperNexus`
