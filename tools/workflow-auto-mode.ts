...
export type WorkflowAutoMode = "off" | "conservative" | "aggressive";

export type WorkflowAutoModeMitigationStatus =
  | "reviewing"
  | "resolved"
  | "needs_changes"
  | "blocked";

export type WorkflowAutoModeRiskLevel = "stable" | "caution" | "severe";

export type WorkflowAutoModeRiskEvaluation = {
  configuredMode: WorkflowAutoMode;
  riskLevel: WorkflowAutoModeRiskLevel;
  reasons: string[];
  riskFingerprint: string | null;
};

export type WorkflowEffectiveAutoMode = {
  configuredMode: WorkflowAutoMode;
  effectiveMode: WorkflowAutoMode;
  riskLevel: WorkflowAutoModeRiskLevel;
  reasons: string[];
  riskFingerprint: string | null;
  mitigationStatus: WorkflowAutoModeMitigationStatus | null;
  mitigationRoundsStarted: number;
  mitigationRoundsRemaining: number;
};

export type WorkflowAutoGateThreshold = {
  avg: number;
  minSingle: number;
};
...
const DEFAULT_THRESHOLD: WorkflowAutoGateThreshold = {
  avg: 8.2,
  minSingle: 7.5,
};

export const DEFAULT_WORKFLOW_AUTO_MODE: WorkflowAutoMode = "off";
...
function normalizeThreshold(
  value: unknown,
  fallback: WorkflowAutoGateThreshold
): WorkflowAutoGateThreshold {
  const record =
...
export function normalizeWorkflowAutoMode(value: unknown): WorkflowAutoMode {
...
export function normalizeWorkflowAutoGateConfig(value: unknown): WorkflowAutoGateConfig {
  const record =
    value && typeof value === "object" && !Array.isArray(value)
      ? (value as Record<string, unknown>)
      : {};
  ...
  return {
    ...
    reviewTimeoutMinutes:
      typeof record.reviewTimeoutMinutes === "number" &&
    ...
    thresholds: {
      ...
      write_to_submit: normalizeThreshold(
        thresholdRecord.write_to_submit,
        DEFAULT_WORKFLOW_AUTO_GATE.thresholds.write_to_submit
      ),
      submit_to_done: normalizeThreshold(
        thresholdRecord.submit_to_done,
        DEFAULT_WORKFLOW_AUTO_GATE.thresholds.submit_to_done ?? DEFAULT_THRESHOLD
      ),
    },
  };
}
...
function includesPattern(items: string[], pattern: RegExp): boolean {
  return items.some((item) => pattern.test(item));
}

function addReason(reasons: string[], message: string) {
...
export function evaluateWorkflowAutoModeRisk(params: {
...
}): WorkflowAutoModeRiskEvaluation {
  ...
  const missingStageSignals = Array.isArray(params.missingStageSignals)
    ? params.missingStageSignals.filter((item): item is string => typeof item === "string")
    : [];
  const reasons: string[] = [];
  let riskLevel: WorkflowAutoModeRiskLevel = "stable";
  ...
  for (const [gateName, rawStatus] of Object.entries(gates)) {
    ...
    if (
      ["blocked", "block", "failed", "fail", "rejected", "reject", "hold", "stopped"].includes(
        status
      )
    ) {
      riskLevel = "severe";
  ...
  const hallucinatedCitationCount = readNumber(
  ...
  if (
    ...
    ["idea", "plan", "code", "experiment", "analyze", "review", "write", "submit"].includes(
      stage ?? ""
    )
  ) {
    if (riskLevel === "stable") {
  ...
  if (
    ["graph_build", "frontier_mapping", "idea"].includes(stage ?? "") &&
    graphPresenceStatus &&
  ...
  if (
    includesPattern(
      missingStageSignals,
      /citation_integrity|verification_status|hallucination|CITATION_VERIFICATION/i
    )
  ) {
    riskLevel = "severe";
  ...
  if (
    includesPattern(
      missingStageSignals,
      /writing_contract\.template_path|appendix_theory|KG_STORYLINE|PAPER_PLAN\.md/i
    )
  ) {
    if (riskLevel === "stable") {
      riskLevel = "caution";
...
export function resolveEffectiveWorkflowAutoMode(params: {
  ...
  manifest?: Record<string, unknown> | null;
  mitigationStatus?: WorkflowAutoModeMitigationStatus | null;
  mitigationRoundsStarted?: number | null;
  mitigationMaxRounds?: number | null;
  riskEvaluation?: WorkflowAutoModeRiskEvaluation;
}): WorkflowEffectiveAutoMode {
  const riskEvaluation =
    params.riskEvaluation ??
  ...
  let effectiveMode: WorkflowAutoMode = configuredMode;
  ...
  if (riskActive && mitigationStatus === "resolved") {
    addReason(
      reasons,
      "Auto discussion panel resolved the current risk, so the configured auto mode is preserved."
    );
    effectiveMode = configuredMode;
  } else if (riskActive && mitigationRoundsRemaining) {
    addReason(
      reasons,
      `Auto discussion rounds remaining before downgrade: ${
        mitigationMaxRounds - mitigationRoundsStarted
      }/${mitigationMaxRounds}.`
    );
    effectiveMode = configuredMode;
    ...
    if (riskActive) {
      addReason(
        reasons,
        "Auto discussion rounds were exhausted without resolving the current risk, so auto mode was reduced."
      );
    }
    effectiveMode =
