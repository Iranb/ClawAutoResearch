...
2. **For EACH paper found** (ALL, not just selected few):
   ...
   - **Step 11:** For one staged paper, queue one workflow-owned upload request that will later execute `pn_import_submit.py` plus `pn_import_queue.py`; for 2 or more staged papers, create one real upload manifest and queue one workflow-owned `pn_batch_import.py` request. Use `research_workflow.queue_paper_ingestion` for this instead of running the upload wrapper inline from Researcher
