...
import { buildWorkflowStatusPresentation } from "../../../tools/workflow-commands/presentation.ts";
import {
...
test("workflow-status presentation exposes Discord controls and leaves local replies plain", () => {
  const snapshot = {
  ...
  const presentation = buildWorkflowStatusPresentation({
    channel: "discord",
    snapshot,
    autoIteratorResult: null,
  });

  const buttonBlock = presentation?.blocks.find((block) => block.type === "buttons");
  assert.ok(buttonBlock);
  assert.deepEqual(
    buttonBlock.buttons.map((button) => button.label),
    ["Refresh", "Resume", "Next", "Graph build", "Handoff"]
  );
  assert.deepEqual(
    buttonBlock.buttons.map((button) => button.value),
    [
      "/workflow-status",
      "/resume-pipeline paper-lab",
      "/monitor-experiment",
      "/graph-build",
      "/handoff-status",
    ]
  ...
  const selectBlock = presentation?.blocks.find((block) => block.type === "select");
  assert.ok(selectBlock);
  assert.ok(
    selectBlock.options.some(
      (option) => option.value === "/capture-diagnostics --reason workflow_status_select"
    )
  );

  assert.equal(
    buildWorkflowStatusPresentation({
      channel: "local",
      snapshot,
      autoIteratorResult: null,
    }),
...
test("workflow-status command returns a readable workflow summary", async () => {
  ...
  const buttonBlock = result.presentation?.blocks.find((block) => block.type === "buttons");
  assert.ok(buttonBlock);
  assert.deepEqual(
    buttonBlock.buttons.map((button) => button.label),
    ["Refresh", "Resume", "Next", "Graph build", "Handoff"]
  );
  const selectBlock = result.presentation?.blocks.find((block) => block.type === "select");
  assert.ok(selectBlock);
  assert.ok(selectBlock.options.some((option) => option.value === "/graph-build"));
