...
import {
  ...
  isExperimentSearchReviewCompleted,
  normalizeExperimentSearchReviewState,
  serializeExperimentSearchReviewState,
  type ExperimentSearchReviewStateLike,
} from "./workflow-guard-state/experiment-search-review.js";

export function getExperimentSearchReviewStatePath(
  projectRoot: string,
...
export async function loadExperimentSearchReviewState(params: {
...
}): Promise<ExperimentSearchReviewStateLike> {
  ...
  const raw = await readJsonIfExists<Record<string, unknown>>(
    getExperimentSearchReviewStatePath(params.projectRoot, configuredPath)
  );
  if (raw) {
...
export async function saveExperimentSearchReviewStateFile(params: {
  ...
  await writeJsonEnsured(
    getExperimentSearchReviewStatePath(
      params.projectRoot,
      params.state.stateFilePath
    ),
    serializeExperimentSearchReviewState(params.state)
  );
}
...
export function buildExperimentSearchReviewSummary(
  state: ExperimentSearchReviewStateLike
): string {
  const bits = [
    `status=${state.status}`,
